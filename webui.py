 ...
 *{box-sizing:border-box}
 ...
 .bagtag{display:inline-block;margin-left:6px;padding:1px 7px;border-radius:9px;
 ...
 tr.qual{background:rgba(14,131,69,.08);box-shadow:inset 3px 0 0 var(--green)}
 tr.qual:hover{background:rgba(14,131,69,.14)}
 ...
 .rbadge{color:#b45309;font-weight:700;font-size:11.5px;white-space:nowrap}
 ...
 .chip .bd.mid{color:#b08900}.chip .bd.old{color:#d62d30}
 ...
 @media(max-width:760px){
  ...
  .fbar input,.fbar select{padding:9px 10px}/* 抽屉内触控目标 ≥38px */
 ...
 @media(max-width:760px){
  ...
  .fbar input,.fbar select{padding:9px 10px}
 }
 ...
 .plitem{padding:9px 12px;border-radius:10px;cursor:pointer;font-size:13px;
 ...
 .plitem.bad{border-left-color:#d62d30}
...
function render(){const s=S;if(!s||!s.users)return;/* 瞬时错误体不级联 */
 setNext(s.next_run);
 $('demoBar').style.display=s.demo?'':'none';
 $('foot').innerHTML=(s.version?'v'+s.version+' · ':'')
  +'<a href="https://github.com/dengmeiluan/ticket-monitoring" target="_blank">GitHub</a> · '
 ...
 (s.users||[]).forEach((x,i)=>{
  ...
  h+=`<div class="ucard"><div class="uhead">
   ...
   <div class="grid">
    <div class="kpi d${xd&&td&&xd.price<=td?' hit':''}"><div class="lab">直飞最低${td?`（线 ￥${td}）`:''}</div>
    ...
    <div class="kpi t${xt&&tt&&xt.price<=tt?' hit':''}"><div class="lab">中转最低·达标班次${tt?`（线 ￥${tt}）`:''}</div>
     ...
     <div class="muted fb">${xt?`${xt.route?xt.route+' · ':''}${xt.name} ${xt.depTime}-${xt.arrTime}${xt.cross?' '+xt.cross:''}${xt.trans?' 经'+xt.trans:''}${xt.bag?' <span class="bagtag">直挂</span>':''}`:''}</div></div>
   </div>
   <div class="muted" style="margin-top:8px">全线最低：${mHtml} ｜ <a href="#" onclick="pickUser(${i});showMonTab('details');return false" style="color:var(--blue)">查看明细 ▾</a></div>
  </div>`;});
 $('users').innerHTML=h;
 countUps();
 CHART_ANIM=1;
...
function expCsv(){const rows=filteredRows();
 ...
 for(const f of rows){
  ...
  `<td>${f.name}${f.bag?'<span class="bagtag">直挂</span>':''}${f.cabinT?`<div class="stl" style="margin-top:3px">${f.cabinT}</div>`:''}${f.view?`<a class="vw" href="${f.view}" target="_blank" onclick="event.stopPropagation()" title="去哪儿查看该航线">↗</a>`:''}${f.route?`<span class="stl"> ${f.route}</span>`:''}</td><td>${f.depTime}</td>`+
  `<td>${f.arrTime}${f.cross?`<span class="stl"> ${f.cross}</span>`:''}</td>`+
  `<td>${f.dur||'—'}</td><td>${f.trans||'—'}${f.layoverT?`<div class="stl lay${(+f.layoverM||0)>=90?' ok':''}">停${f.layoverT}</div>`:''}</td>`+
...
function chart(){if(!S||!S.users)return;/* 状态未到不画（render 会重调） */
 ...
 const W=c.offsetWidth,H=280,L=64,R=14,T=14,B=52;
 const H0=CR.range==='7d'?(CR.r.history7||CR.r.history):CR.r.history;
 console.log('[chart-dbg]','range='+CR.range,'h7='+(!!CR.r.history7),'h7N='+((CR.r.history7&&CR.r.history7.direct)||[]).length,'histN='+((CR.r.history&&CR.r.history.direct)||[]).length,'offw='+c.offsetWidth,'Susers='+(S.users&&S.users.length));
 ...
 if(NT&&tickT(0)){
  ...
  for(let i=1;i<NT;i++){
   ...
   if(day!==prevDay){
    ...
    ctx.fillStyle='#9a7b30';ctx.fillText(day,dx,H-8);
    prevDay=day;}}}
 ...
 if(K){
  ...
  CT.forEach((c,i)=>{if(!best||c.l<best.c.l)best={c,i,s:'t'};});
   if(best&&best.i!==(best.s==='d'?CD:CT).length-1){
    ...
    const kx=Math.min(Math.max(bx+6,L),W-kw-6),ky=Math.max(Y(best.c.h)-8,T+12);
    tagBox(kx,ky,kw);
  ...
  const series=(pts,color,tv)=>{if(!pts.length)return;
   ...
   if(tv>0)/* 达标数据点：白心绿环旗标——只画入场点（首次跌破）与最新点，
      长期达标段不再逐点密铺成串（7 天窗口实测） */
    pts.forEach((p,i)=>{const hit=p[1]<=tv,
     prevHit=i>0&&pts[i-1][1]<=tv;
     if(hit&&(!prevHit||i===pts.length-1)){
  ...
  const markMin=(pts,color)=>{if(!pts||pts.length<3)return;
   ...
   const tx=Math.min(Math.max(x+6,L),W-tagW-6),ty=Math.max(y-10,T+12);
   tagBox(tx,ty,tagW);
...
function renderCal(){if(!S||!S.users)return;/* 状态未到不画 */
 ...
 for(const pair of cal){const d=pair[0],v=pair[1];
  ...
  const best=v>0&&v===mn;
  const tip=v>0?(th>0?(v<=th?d+' 直飞最低 ￥'+v+' · 已达标 ✅':d+' 直飞最低 ￥'+v+' · 超线 '+Math.round((v/th-1)*100)+'%'):d+' 直飞最低 ￥'+v)+(best?' · 近期最低':''):(d+' 无数据');
  h+='<div class="calcell'+(best?' best':'')+'" style="background:'+bg+';color:'+fg+'" title="'+tip+'">'
   +'<div class="cd">'+d+'</div><div class="cp">'+(v>0?'￥'+v:'—')+'</div>'
 ...
 h+='<div class="rline" style="display:flex;align-items:center;gap:8px">'
  ...
  (u.routes||[]).forEach((r,j)=>{
   h+=`<div class="rline${r.enabled===false?' off':''}" data-rk="${i}:${j}">
    ...
    +`<input type="checkbox" class="switch" title="启用/停用该航线：停用保留全部配置，调度跳过，随时可恢复" ${r.enabled===false?'':'checked'} onclick="event.stopPropagation()" onchange="setRouteEnabled(${i},${j},this.checked)">`
    +`<button class="danger" onclick="delRoute(${i},${j})">✕ 删除</button></div>
    ...
    <div class="srow"><div class="slab"><b>🧳 中转行李直挂</b><div class="shint">仅认渠道标注「中转行李免提」（两段含免费托运）的班次，未标注的视为不满足</div></div><div class="sctl"><input type="checkbox" class="switch" ${r.transfer_baggage==='direct'?'checked':''} onchange="setRoute(${i},${j},'transfer_baggage',this.checked?'direct':'')"></div></div>
 ...
 *{box-sizing:border-box}
 ...
 @keyframes hitpulse{0%,100%{box-shadow:0 0 0 0 rgba(14,131,69,.4)}
      50%{box-shadow:0 0 0 5px rgba(14,131,69,0)}}
...
def _user_state(u):
    ...
    hist = _rounds(db, route.get("from", ""), route.get("to", ""),
                   ...
                   need_direct=route.get("transfer_baggage", "") == "direct")
