...
| 聚合推送 | 多航线一轮一消息；**达标航线小节自动置顶**；KPI 三行制 + 进度条（**破线满格绿 🟩**）+ **💡 操作建议** + **📉 7 天趋势** |
...
| **可视化对齐** | 网页走势与推送图表同一信息密度：X 轴日期时间刻度、跨天分隔线、**达标数据点白心绿环旗标**、最低点「哪天几点」标注；推送明细总表圆角白卡+分组色带+类别胶囊+**比价两行式行卡（整链零截断）** |
...
```bash
python tests/test_core_units.py   # 纯函数单测（41 用例）
