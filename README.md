...
│    > 💡 直飞超线 30%，继续观望
│    > 📉 近7天 直飞 ↓8% ｜ 中转 ↓3%
│    [价格走势折线图：●实绿=真达标 ○细绿=行情破线 ●琥珀=距线≤10%]
...
> 标题全 `####` 档、语义点分档主次分明（🎯达标 🟩破线 🟨擦边，超线为默认态
...
| 聚合推送 | 多航线一轮一消息；**达标航线小节自动置顶**；**KPI 语义三行制**（价格加粗直达链接 / 航班正文 / 决策字段引用辅档）+ **语义点分档**（🎯达标 🟩破线 🟨擦边，超线为默认态不占点）+ **💡 操作建议** + **📉 7 天趋势**；全行手机行宽估宽守卫（超宽逐级降级，恒保价格/航班/判据）；达标明细行「打开渠道」一键跳转；通道故障自动在文末警示（告警系统自身的故障必须被看见） |
...
| 走势图 | 平滑曲线+渐变面积/**K线双模**（涨红跌绿、OHLC 悬停）；48h/7 天范围；最低点「哪天几点」标注；**达标点环三档**（实心绿=真达标 / 空心绿=行情破线 / 琥珀=距线≤10%）与文案语义档、明细表 🔥 三方同语言 |
...
| 明细表 | 筛选/排序/日期列/搜索/**达标行浅绿底+绿条**/近达标琥珀/同班比价展开/CSV 导出/每行直达下单页 |
...
| 可靠性 | 调度互斥锁、钉钉 -1 单发零重试、渠道失败可见、启动首轮节流；**用户跳转链接与爬虫主路径同源 CI 门禁**（渠道改版即测试红，杜绝点开死页） |
...
全部配置在网页 ⚙️ 配置页完成，**保存即热生效（零重启）**：航线/阈值/出发时段、钉钉/Server酱微信/ntfy/阿里云通知、扫描周期与扰动、控制台端口（原地切换）、无头浏览器、采集超时/错峰间隔、每日日报时刻、图床。`config.yaml` 只是落盘格式；仅数据库/日志路径为启动级（见 `config.example.yaml`）。
...
```bash
PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 python -m pytest tests/ -q   # 全量 154 项：单测（含 tuniu 舱位 5 / 哨兵扩容 / 飞猪幻影价 MF2372 守卫 / 跨渠道字段补全 / nid 死链回环）+ 推送链路演练 + 链接同源 + 版本门禁
python docs/uitest.py             # 全功能 UI 自检（108 断言，自动拉起演示控制台）
