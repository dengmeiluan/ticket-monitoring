...
# 锚定仓库 data 目录（与 main._SENT_PATH 同族收口）：CWD 相对路径禁用
# ——进程 CWD 漂移（计划任务/手动启动差异）=脉冲历史清零/
...
class Pulse:
    ...
    def _load(self):
        """启动恢复：载入全部结构完好轮次（缺失/损坏静默从零开始）。
         移除按属主 pid 过滤——用户实报「脉冲没有历史记录」：新
        进程 pid 必异于旧轮次，过滤即每次重启历史清零，与「跨重启连续
        可读」产品意图冲突。当年防的双实例互污染已由单实例互斥锁
        （main.acquire_instance_lock）在写侧根治：锁拒绝第二实例并存，
        不存在「另一实例的轮次」可污染。结构校验保留（损坏/异形丢弃）。"""
        try:
