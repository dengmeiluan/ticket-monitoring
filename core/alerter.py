...
class Alerter:
        ...
        try:
            if self.storage:
                ...
                hist = _rounds(self.storage.db_path, route.from_code,
                               ...
                               need_direct=route.transfer_baggage == "direct")
                out = {}
    ...
    def _digest_payload(self, rs_list: list, fresh: bool = True,
        ...
        if hits:
            ...
            title = (f"{prefix}{self._route_short(r0)} "
                     f"{self._date_short(s0['date'])} {k0}￥{f0['price']}"
        ...
        # 进度条图例放头部：读者第一眼见到 🟦🟦⬜⬜⬜ 时就能对上含义
        # （原在全文末尾，等读完所有航线才出现=无人看到）
                ...
                for s in sections:
                    ...
                    if cands:
                        ...
                        for save, fs2 in cands:
                            ...
                            # 结构化行（非预拼文本）：明细总表按三列表格渲染，
                            # 预拼字符串曾把 ⚖️ 带进 PIL 渲成「口」方块
                            comp.append({
                                "label": label,
                                "dep": best["depTime"], "arr": best["arrTime"],
                                "trans": best.get("transCity") or "",
                                "chain": [(self.PLATFORM_CN.get(
                                            f.get("_platform", ""), "?"),
                                           f["price"]) for f in fs2],
                                "save": save})
    ...
    def _gauge(price: float, threshold: float, bars: int = 5) -> str:
        ...
        价格越接近达标线 🟦 越少；已破线则 🟩 满格——全空格曾像「没数据」，
        满格绿一眼传达「可出手」。（▓░ 在钉钉会渲染成黑块，弃用；
    ...
    def _transfer_ok(cls, f: dict, route) -> bool:
        ...
        layoverM/transferBaggage 缺失时对应维度自然退化为不限制。"""
        def _rk(key, default):
