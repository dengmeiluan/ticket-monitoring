...
# 兜底：搜索运行中的 main.py 进程（锁文件缺失或指向已死 PID 时，
# 真实监控进程可能仍在运行——只信锁文件会漏杀成僵尸实例）
function Find-MainProcs {
    Get-CimInstance Win32_Process -Filter "Name='python.exe' OR Name='pythonw.exe'" |
        Where-Object { $_.CommandLine -like "*main.py*" }
}

function Stop-FoundProcs {
    $procs = Find-MainProcs
    if ($procs) {
        ...
        Write-Host "未发现运行中的监控进程" -ForegroundColor Gray
    }
}
...
if ($proc) {
    ...
    Write-Host "PID=$pid_ 进程不存在，锁已过期" -ForegroundColor Yellow
    Stop-FoundProcs
}
