...
- **Watches** your routes with per-route thresholds (direct / transfer), arrival-time constraints and departure-time windows. Every route has an **enable/disable switch** — pausing keeps the whole config and exempts the route from scheduling, pushes and details everywhere; no more delete-to-stop. A **daily report** lands at your configured hour (default 09:00) with per-route KPIs and trend images.
...
```bash
python tests/test_core_units.py   # unit tests (49 cases)
python tests/test_alert_path.py   # push-pipeline dry run
