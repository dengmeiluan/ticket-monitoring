...
def test_qunar_pc_parse_direct_and_transfer():
    ...
    # PC transTime 语义=停留时长（19小时25分钟=起降差真值）；段级无
    # flightTime→全程重建不可为留空，normalize 按起止时刻重算兜底
    assert t["totalDuration"] == "" and t["layover"] == 1165
    from core.flightnorm import normalize as _nf
    _t = _nf(dict(t), "2026-09-25")
    assert _t["totalDuration"] == "25时30分" and _t["durM"] == 1530
    assert _t["layoverM"] == 1165 and _t["layoverSrc"] == "times"
...
def test_fliggy_dual_price_rows_take_low():
    ...
    # 取低者为主价。：_alt_price 留痕内部键改清理不进 extra
    # （无任何消费端，泄漏卫生修复），测试改锁「不落库」语义
    txt = ("国航CA8564\n14:30\n18:45\n¥9900 全价\n¥3960 4.5折\n订票")
    fs = FliggyCrawler._parse_pc_text(txt, "2026-10-06")
    assert len(fs) == 1
    assert fs[0]["price"] == 3960.0
    assert "_alt_price" not in fs[0]
...
def test_daily_minima_qual_flag():
    ...
    # -1=擦边（线<价≤线×1.1， 补档与走势环/表格图三媒质同语言）
    # 0=线外
...
def test_rounds_transfer_uses_normalized_layover():
    ...
    否则 layoverM 恒缺、中转序列恒空（中转走势线消失根因）。"""
    import sqlite3 as _sq
    import tempfile as _tf
    import json as _j
...
def test_suggest_line_breakthrough():
    ...
    # 措辞收口：建议行不再说「破线」（图例 🟩破线=行情破线
    # 未必达标，一词两义曾误读）； 改 25-30 半角短式
...
def test_suggest_line_near_and_far():
    ...
    # 超线态返回空：与 KPI 差额、仪表条三重冗余零增量，撤行减段
    assert far == "", far
...
def test_pulse_history_survives_pid_change(tmp_path):
    """真重启恢复（用户实报：脉冲只有本轮 1 根柱）——旧轮次
    pid≠新进程 pid，载入侧按属主 pid 过滤曾把历史整体丢弃（每轮重启
    即清零，与「跨重启连续可读」产品意图冲突）。单实例互斥锁
    （main.acquire_instance_lock）已承担双实例防污，pid 过滤冗余。
    红→修→绿：落盘轮次 pid 改成他进程号，新实例必须保留。"""
    import json
    from core.pulse import Pulse
    st = str(tmp_path / "pulse.json")
    p = Pulse(maxlen=6, store=st)
    p.begin(); p.channel("qunar", 70, 8.2); p.end()
    p.begin(); p.channel("ctrip", 33, 4.4); p.end()
    # 模拟重启：落盘文件里历史轮次属「上一个进程」（pid 必然不同）
    rounds = json.load(open(st, encoding="utf-8"))
    assert len(rounds) == 2
    for r in rounds:
        r["pid"] = (os.getpid() + 7777) % (2 ** 22)
    json.dump(rounds, open(st, "w", encoding="utf-8"), ensure_ascii=False)
    q = Pulse(maxlen=6, store=st)
    v = q.view()["rounds"]
    assert len(v) == 2, f"重启后历史应保留,实得 {len(v)}"
    assert [r["rows"] for r in v] == [70, 33]
    # 损坏轮次仍被拒（结构校验保留）
    rounds.append({"no_ts": True})
    json.dump(rounds, open(st, "w", encoding="utf-8"), ensure_ascii=False)
    q2 = Pulse(maxlen=6, store=st)
    assert len(q2.view()["rounds"]) == 2
...
def test_daily_report_desp_structure(monkeypatch, tmp_path):
    """每日日报 desp 升级：头部数据截至行 + 每航线 KPI 摘要
    ...
    # 头部行守 20 全角红线：短时刻 + 口径窗（旧「数据截至」
    # 静态拼接曾 43 半角必折行）
    assert "> ⏱ " in d and "近2.5时口径" in d, d[:80]
    # KPI 图内化：行情数字撤文本、进明细表图 summary（定律：
    ...
    # 超线默认态不加点（蓝色噪音），图例改「无标超线」
    assert "🟦" not in d, d[:200]
    # 头部图例行撤入走势图（图内点环分档小注同款，双份冗余）；
    # 头行短式化，「档位图例见图内」与图内小注双份冗余一并删
    ...
    assert "生成于" not in d
    # 「🔍 打开渠道查现价」链接行真实产出（恒
    # TypeError 被裸 pass 吞、72 条日报 0 次），按设计落小节尾=末行；
    # 与明细表图之间是段落级空行，取非空行对账
    assert lines[-1].startswith("[🔍 打开") and "](" in lines[-1], lines[-1]
    tail_ne = [l for l in lines if l.strip()]
    assert tail_ne[-2].startswith("![明细表]"), tail_ne[-2]
...
def test_gauge_full_green_when_hit():
    ...
    assert g(2319, 1600) == ""            # 超线撤仪表条（行1 差额%已表达）
    assert g(1850, 1600) == ""            # 超线 15.6% 同撤
    assert g(2000, 0) == "" and g(None, 1600) == ""   # 无阈值/无价不渲染


def test_digest_table_compare_rows_structured():
    """明细总表比价行结构化：预拼「⚖️ …」字符串曾把 emoji
...
# ---- 回归：urgent 通道 NameError 哑弹（_re 只在 _plain 局部导入，
...
# ---- 告警通道自监控：_send_urgent 返回成败，失败须浮出水面 ----
...
# ---- 日期 chips：saveCfg 对 dates 的校验保持兜底（导入配置仍过检） ----
...
# ---- 回归：页脚版本号脱节（tag 已到 而 __version__ 仍，
...
# ---- 回归：停留无限逼近全程的擦边毒值（9C8846 全程7h55 停7:55，
...
# ---- 回归：qunar H5 假衔接（binfo2=binfo1 复制）判伪守卫 ----
...
# ---- 回归：tuniu 多段 offer 明细仅首段口径 → 显式跳过 ----
...
# ---- 回归：同程键名改版（cabinlevel/equipmentName 消失） ----
...
def test_digest_legend_four_tiers_all_fit():
    """图例四档（起断言现役 _legend_line）：🎯 基座恒在、档序与
    ...
    assert _dw(leg) <= 38, leg


def test_suggest_line_overline_returns_empty():
    """：超线默认态建议行返回空（与 KPI 差额、仪表条三重冗余
    撤除）——最近档超线⇒全部超线⇒整行零增量。"""
    import logging as _lg
    a = Alerter(_lg.getLogger("t"), storage=None)
...
def test_top3_blocks_fire_uses_qual_price():
    ...
    让低价亮 🔥 而图/明细不绿。pad=0（重标定）下 qual==展示价，
...
def test_cross_source_time_arbitrage_skips_transfer_chains():
    ...
    按段匹配不得让第二段时刻覆盖整行起降（防假数据直入推送/指纹）。"""
    import logging as _lg
    from core.alerter import Alerter
...
# ---- 回归：超线去蓝点/强提醒过滤修复/幽灵送达落账/表格次行零省略 ----
...
def test_daily_report_ghost_errcode_marks_state(monkeypatch, tmp_path):
    ...
    (tmp_path / "data").mkdir()
    # 状态文件锚定 _ROOT：测试 patch 到 tmp（chdir 已不再影响
    # 落点，保留只为兼容其他相对路径读侧）
...
def test_field_sentinel_warns_on_dead_field(monkeypatch, tmp_path):
    ...
    命中正常或行数不足不报。门槛 50→20：tongcheng 48 /
    ...
    # cabin/meal/prate/plane 四字段全灭（补 plane）；行李直挂
    # 分母改中转行——夹具无中转行不再触发直挂告警。
    # 同程 term 入白名单（航站楼 dat/aat 47/47 实证）0 命中
    # 同为键死信号 → 4+1；svc（transferService 只挂中转行、DB 实测
    # 同程中转记录仅 7.3%）有 trans_n≥20 分母门槛——夹具无中转行不报。
    # 第七批：psize/apt 入比率观测（首组循环，无白名单）+2；
    # tongcheng|avgd/cancel/bridge/page 入白名单恒 0 观测 +4；
    # aptc（IATA 码覆盖率）入比率观测 +1；discount 比率观测 +1 → 13
    assert len(seen) == 13, seen
    _m._field_sentinel([_NS(platform="tongcheng", extra=json.dumps(rows))], _L())
    assert len(seen) == 13, seen          # 同日再跑不刷屏
    ok_rows = [{"price": 1000, "cabin": "经济舱", "meal": "无餐食",
                "prate": "97", "plane": "空客321(中)",
                # shareCarrier/arrTerminal 按生产形态给值（哨兵
                # 0 命中=键死观测；健康夹具须带命中——share DB 实测
                # 20.6%、arr 侧航站楼 98/98）；avgDelay 同（                # ctrip|avgd 白名单，extendinfos 近全航班在场）；
                # leftTickets/fewTicket/transTerminal 同（白名单，
                # quantity 148/148、.less-tag 26/32、中转航站楼 88% 中转行）；
                # discount 同（比率观测，「全价」并入后五渠道恒有源）
                "discount": "4.5折",
                ...
                "layover": 90,   # transCity 在场→trans_n=60，停留须带值防 <10% 告警
                "transferBaggage": "direct" if i < 2 else "",
                # 第七批：psize/apt 比率观测 + ctrip|bridge/
                # page/ccode 白名单恒 0 观测——健康夹具带命中值；
                # aptc 比率观测同律
                "planeSize": "中型机", "depAirport": "咸阳",
                "depAirportCode": "URC", "arrAirportCode": "SHA",
                "bridgeRate": 80, "planeAge": "6.3", "cabinCode": "Y"}
    ...
    # fliggy|meal 入死键表不再报（结构性无源）；shareCarrier
    # 夹具带值（0 命中=键死观测，健康夹具须带命中，DB 实测 40.9% 同量级）。
    # psize/apt 比率观测入首组循环——fliggy 生产形态 100% 在场，
    # 夹具带值防假火；prate/plane+fewTicket 共 3 不变
    mid = [{"price": 1000, "cabin": "", "meal": "", "prate": "",
            "shareCarrier": "CZ6983" if i % 3 == 0 else "",
            "arrTerminal": "T2" if i % 2 == 0 else "",
            "planeSize": "中型机", "depAirport": "天山机场",
            "discount": "4.5折"}   # 比率观测带命中值防假键死
           for i in range(26)]
    _m._field_sentinel([_NS(platform="fliggy", extra=json.dumps(mid))], _L())
    # fliggy|few 入白名单（.less-tag 26/32 在场）恒 0 命中同为
    # 键死信号 → prate/plane+fewTicket 共 3（meal 已入死键表；cabin
    # 随调研⑤ DB+页面双实证「结构性页面无源」迁入死键表）
    assert len(seen) == 3, seen


def test_field_sentinel_layover_and_rowfall(monkeypatch, tmp_path):
    """哨兵扩容：中转行停留覆盖率（ctrip 单点失效→全链停留静默
...
def test_ctrip_cross_day_by_date_diff():
    """ctrip 跨天按日期差：时长//1440 映射曾把 ≥3 天一律
    标「+2天」——日期字段是唯一事实源。"""
...
def test_suggest_line_short_forms_within_width():
    """建议行改短式：常见形态本就 41-48 半角、_fit_line 末档
...
def test_split_market_pool_writes_th_amber_tier():
    """日报表琥珀擦边档：_split_market_pool 曾从不写 _th，
...
def test_archive_nid_survives_sanitizer():
    """nid 死链修复：hex 小写字母曾被 webui [^A-Z0-9] 净化
...
def test_low_txt_tax_note_conditional():
    """差额文案「(税前)」单源 _low_txt：税前注仅 pad>0 且
...
def test_midnight_window_warns_once(monkeypatch):
    """跨零点出发窗口：dmin>dmax 的钟面窗口判定恒 False，
...
def test_near_ratio_single_source_and_warn_sync():
    """擦边带宽单源与琥珀对比度三端同值：×1.1/≤0.10 字面量
    收敛 report.py NEAR_RATIO（webui 侧 TIER/fl/near 同步注释）；
    C_NEAR 加深 (176,137,0)→(154,120,0) 后 webui --warn 浅色同步
    #9a7800；再加深→(138,108,0) 同步 #8a6c00 过 AA
    （白卡 4.97:1、斑马行 4.65:1），暗色 #d9b34a 两侧不动。"""
    import io
    import os
    import report as _rep
    ...
    assert C_NEAR == (138, 108, 0)
    src = io.open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                               os.pardir, "webui.py"),
                  encoding="utf-8").read()
    assert "#8a6c00" in src, "webui --warn 浅色未随 C_NEAR 同步（三端同值纪律）"

def test_v1546_price_color_brk_hollow_tier():
    """总表图破线未达标档：价≤线但口径不符 → C_QUAL 空心
...
def test_v1546_near_ratio_single_source_alerter():
    """擦边带宽收口 core.alerter 单源：report/webui 后端一律
...
def test_v1546_compare_rows_shared_assembly():
    """compare 组组装单源 _compare_rows：实时总表与日报共用
...
def test_v1546_cfgnav_specificity_fix():
    """配置左导航特异性修复：.cfgnav>span 通配曾以 (0,1,1)
...
def test_sentinel_layover_dual_key_and_median_ratio(monkeypatch, tmp_path):
    """哨兵双键兼容（P0）：原始 extra 键是 layover（layoverM 是
    normalize 产物）——恒 0 命中=健康渠道天天假告警；渠道中位比
    观测对系统性漂移渠道告警。"""
    import json as _json
    import logging as _lg
    import types
