...
def test_daily_report_desp_structure(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(_rep, "_route_latest_flights",
                        lambda db, fc, tc, d: flights)
    monkeypatch.setattr(_rep, "render_flights_table", lambda *a, **k: None)
    ...
    d = sent["desp"]
    assert d.startswith("# 📈 每日走势与明细"), d[:60]
    assert "> ⏱ 数据截至" in d, d[:80]
    assert "直飞最优 ￥2,690" in d and "超线 ￥690" in d, d[:300]
    assert "中转最优 ￥2,150" in d and "超线 ￥350" in d, d[:300]
...
def test_digest_table_compare_rows_structured():
    ...
    def fake_render(rows, title, out, top_n=5, summary=""):
        captured["rows"] = rows
    ...
    assert row["chain"] == [("去哪儿", 3396), ("飞猪", 7870)]
...
def test_flightnorm_layoverM_and_baggage_default():
    ...
    f = norm({"transCity": "西安", "layover": 75, "depTime": "22:00",
              "arrTime": "23:30", "depDate": "2026-09-25",
              "arrDate": "2026-09-25", "price": 2000})
    assert f["layoverM"] == 75 and f["layoverT"] == "1时15分"
