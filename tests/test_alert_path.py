...
def main():
    ...
    # ---- v1.5.31 语义分档深化回归：行宽守卫/降档/链接/多日期 ----
    ...
    # 同班比价文本段已撤（v1.5.36 图入库：无跳转需求的内容只出图）
    ...
    _rep.render_flights_table = lambda rows_, title, out, summary=None, \
        stamp_note="": (_cap.update(rows=rows_), out)[1]
    _rep.upload_freeimage = lambda path, logger=None: "https://iili.io/fake.png"
    try:
