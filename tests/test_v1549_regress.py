# -*- coding: utf-8 -*-
"""v1.5.49 回归测试：每个用例对应本轮五路调研修掉的一个真实缺陷。
...
def test_suggest_line_danstalk_market_base():
    """蹲守档与行情同基（v1.5.43 定律补完实现）：判定用行情价、金额
    曾用达标口径价——同消息 KPI 行1「低￥10」与建议行「差￥50」两数
...
def test_field_sentinel_dead_keys_skipped(monkeypatch, tmp_path):
    """死键表（qunar|prate、qunar|plane、fliggy|meal）：现役路径无源，
    继续观测=健康渠道天天假告警（v1.5.49 键名三方对账定论）。夹具
...
def test_field_sentinel_no_detail_rows(monkeypatch, tmp_path):
    ...
    此前对字段/塌方/脉冲三面隐形。1 条空记录是常态波动不入报。"""
    from types import SimpleNamespace as _NS
    import main as _m
    monkeypatch.setattr(_m, "_SENT_PATH", str(tmp_path / "sent.json"))
    monkeypatch.setattr(_m, "_ROW_HIST", {})
...
def test_flights_table_full_fields_capacity(tmp_path):
    """v1.5.48 全字段行（机型·舱位·折扣·准点·餐食·托运·共享·余票）
    ~489px 曾击穿两行容量 → plan=None 整行省略（决策字段零省略破口）。
    v1.5.49 优先级重组（丢折扣→机型重试拆行）后渲染必须成功且行高
    只增 16（拆两行），不得整行回退省略。"""
...
# ---- v1.5.50：钉钉 -1 未达标心跳退避（HANDOFF §9.3 落地）----
...
# ---- v1.5.50：渠道决策字段扩采（trendGo/labels/童婴价）----
