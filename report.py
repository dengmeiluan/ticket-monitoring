...
from core.alerter import (Alerter, _cabin_cn, _qual_price, FLIGGY_TAX_PAD,
                          ...
                          NEAR_RATIO, gap_txt, kpi_tier_txt)
from core.flightnorm import normalize

# 仓库根锚定（v1.5.49 CWD 漂移免疫家族）：config 相对路径与状态文件的
...
C_TH_T = (138, 109, 31)       # 中转达标线 棕金（--thline #8a6d1f 同值）：
                              ...
                              # 红金双色早已分档，v1.5.41 对齐同语言）
C_GRID = (225, 228, 232)
C_TEXT = (60, 64, 70)

# 擦边带宽 NEAR_RATIO 已收口 core.alerter 单源（v1.5.46：本文件本地
...
def _rounds(db_path, from_city, to_city, date, arrival_max, hours=48,
    ...
    中转序列恒空（v1.5.2 根治「中转走势线消失」）。行李直挂约束不参与
    ...
    走势绿环据此只标「真达标」点，行情破线未达标点画细绿环区分
    （与明细表「价格绿=达标」同一语义，v1.5.32 口径收口）。
    ...
    for r in rows:
        ...
        for f in obj:
            ...
            try:
                ...
                continue
            # 价格带与爬虫端同带（300–50000，v1.5.47 取数链自设防）：
            # 此前纯靠爬虫端过滤，任何漏网毒行（0 价/异常值）直入
            # 曲线/日历口径
            if not 300 <= p <= 50000:
                continue
    ...
    for ts, ds, tsf in buckets:
        d = t = None
        qd = qt = False
        # 跨渠道孤低价守卫（v1.5.45，与告警池同一 xchan_phantom_idx）：
        ...
        if tsf:
            # 同指纹衔接补全先于 layover_min 过滤（v1.5.43 桶内就地）：
            # 与明细表/推送池同预处理——qunar 中转行 90% 缺停留时长，
            # 不补全则被 layover_min 整行剔除、曲线比列表偏高（实锤
            # 同轮 2164 vs 2190 两价的曲线侧残留病灶，曲线与列表含义
            # 一致的最后一块拼图）。只补本桶行，不触碰历史
            Alerter._propagate_layover([f for f, _p, _pl in tsf])
            ok_t = [(f, p, pl) for f, p, pl in tsf
                    if layover_min <= 0
                    or (isinstance(f.get("layoverM"), int)
                        and f["layoverM"] >= layover_min)]
            # 中转序列同守卫（与直飞同一独立锚语义）
            ok_t = [x for i, x in enumerate(ok_t)
                    if i not in xchan_phantom_idx(
                        [(p + (FLIGGY_TAX_PAD if pl == "fliggy" else 0), pl)
                         for _f, p, pl in ok_t])]
            if ok_t:
...
def _daily_minima(db_path, from_city, to_city, date, arrival_max, days=14,
    ...
    -1=擦边（线 < 价 ≤ 线×(1+NEAR_RATIO)，v1.5.40 补档——与走势环/表格图三档
    同语言，此前日历无擦边档）、0=线外。曾返回 {0,1} 布尔旗标而
    消费端按三档解码——日历上「真达标」永不出现、达标日反被标
    「行情破线，未必可出手」。数据窗口受存储清理限制（14 天）。
...
# v1.5.38 航班列加宽 16 后表格 936、走势仍 920 曾破同宽不变量，v1.5.41
# 走势侧跟齐）。v1.5.43：全程列 78→88——「13时40分」5 字形态在 78px
...
# C_HEAD_BG/C_ROW_ALT 死令牌已删（v1.5.37 收敛）：值是 v1.5.33 色值对齐
...
C_NEAR = (154, 120, 0)         # 擦边=价距线≤10% 琥珀（走势环/表格图价格/日历
                               # 三媒质同值同义；与衔接警示分色 v1.5.40——原
                               # 与 LAY_SHORT 同值两义，同图琥珀只表擦边）。
                               # v1.5.44 加深 (176,137,0)→(154,120,0)：
                               # 白底对比 3.3:1→4.2:1（原值在走势环 13px
                               # 小注上不达 4.5:1 大字线；webui --warn 浅色
                               # #9a7800 同步改值，暗色 #d4a72c 不动）
...
class DESIGN:
    ...
    v1.5.33 起色值实测对齐 :root（此前注释声称同值实际漂移：
    BG/蓝/红/橙/MUTED 四处偏差）。"""
    ...
    LAY_SHORT = (192, 86, 26)       # 衔接偏短警示 暗橙红（原 (176,137,0)
                                    # 与擦边琥珀同色两义，v1.5.40 分色；webui
                                    # --stl 同步改值——两处同值同注释纪律）
    CHAIN_TX = (64, 76, 92)         # 比价渠道链
    TIP_LINE = (214, 222, 232)      # 空态卡/提示框描边（v1.5.44 收编：曾
...
def _fit_text(d, text, x, y, max_w, base=15, fill=C_TEXT, min_size=11,
    ...
    anchor 直传 PIL（v1.5 起行内元素统一 "lm" 锚行中线）。"""
    txt = str(text)
...
# ——曾整段入类被剥成「上乌」「08:3013:50」（v1.5.37 修正）
_EMJI = re.compile(
    "[🀀-🫿☀-➿🇦-🇿"
    "️]+")
...
def _price_color(f: dict, qual: bool, price: float):
    ...
    线×(1+NEAR_RATIO)）/ 超线深蓝——与走势环同档同色（此前表格图两档
    缺擦边，同一次推送两种分档语言）。擦边区间下界严格 >th（Soldier
    审查收窄，与 webui near 同式）。阈值由调用方写 f["_th"]，缺省退
    两档兼容旧调用。
    破线未达标（价≤线但口径不符：飞猪税前/中转衔接欠）曾落入超线同款
    深蓝——同表「更便宜反默认色」梯度倒挂；v1.5.46 归 C_QUAL 空心
    （_price_hollow），与走势环「实心=达标/○细绿=行情破线」同构：
    同色系、空心=行情口径未必可出手。v1.5.47 档位结构收 _tier_of 单源。"""
...
def _summary_join(measure, bits, budget):
    """summary 宽度预算逐条取（v1.5.49）：调用端 [:4] 静态截断曾把
...
def render_flights_table(rows, title, out_path="data/flights_table.png",
    ...
    plat_mins（{platform: 最低价}——组尾「各渠道最低」子节，v1.5.40
    自钉钉文本段撤入）、
    ...
    def _plan_sub(f):
        ...
        v1.5.49 容量重组：全字段行 ~489px 曾击穿两行容量（无徽标上限
        ...
        def _parts():
            ...
            return [(k, t) for k, t in (
                ...
                ("cabin", _cabin_cn(f.get("cabin"))),
                # 折扣/托运额入图（v1.5.40 决策信息补全）：折扣只认「N折」
                # 形态防脏值长文；托运额是比价隐性成本（廉航对比显式更关键）
                ("discount",
                 (str(f.get("discount") or "").strip()
                  if re.fullmatch(r"\d+(?:\.\d+)?折",
                                  str(f.get("discount") or "").strip())
                  else "")),
                ...
                # 共享实际承运/余票标签（v1.5.48 决策字段入图）：共享=营销
            ...
            def _rebuild(drop, short_share):
                ...
                for k2, t2 in kvs:
                    ...
                    if short_share and k2 == "share":
                        m = re.search(r"([A-Z][A-Z0-9]*\d{3,4})$", t2)
                        if m:
                            t2 = "共享·" + m.group(1)
                    out.append((k2, t2))
                return out

            for _drop, _ss in ((("discount",), False),
                               (("discount", "plane"), False),
                               (("discount", "plane"), True),
                               (("discount", "plane", "cabin"), False),
                               (("discount", "plane", "cabin"), True)):
    ...
    # 图例条件与 meta 解耦（v1.5.37）：旧条件 meta_any and it_has_transfer
    # 在告警路径（rows 无第 4 元 meta）恒 False→整图无图例，但图上照样有
    # 达标绿行/「可省/差」/直挂徽标需要解码——改按内容扫描：达标行/
    # compare 组/transfer 组任一出现即显示（图例内容渲染逻辑不变）。
    # v1.5.44 补琥珀判据：纯直飞小节出现琥珀擦边价时曾 legend_h=0，
    # 图内琥珀价无任何解码词条（与 v1.5.43 补橙红词条同型病灶）——
    # 任一行价格着非默认色即 worthy
    def _colored(f):
        try:
    ...
    legend_worthy = (
        ...
        or any(_colored(f) for it in rows if it[0] != "compare"
               for f in it[1] if isinstance(f, dict)))
    # summary 预排（v1.5.49 两行式）：列表入参按宽度预算逐条取行，
    ...
    legend_h = 48 if legend_worthy else 0   # 两行（v1.5.43 补橙红词条后
    ...
    f_title, f_head, f_cell, f_grp = (d.font(21), d.font(16),
                                      d.font(16), d.font(19))
    # 顶栏标题/时间戳统一 anchor 中线锚（v1.5.37 收口，同锚 y=30 行中线）。
    # 标题限宽（v1.5.43）：多航线长城市对裸落曾右溢压时间戳，与走势侧
    # _fit_text（890）对称
    ...
    y = 62
    if _sm_lines:
        # 摘要盒（v1.5.49 多行式）：行数决定盒高，anchor 逐行均分盒高
    ...
    for item in rows:
        ...
        if kind == "compare":
            # compare 区块同样遵守统一 anchor 中线纪律（v1.5.37 收口）。
            ...
            for line in fs[:top_n]:
                ...
                if isinstance(line, str):      # 旧调用方：预拼文本直接落
                    ...
                    # y+18（v1.5.37 收口）；右缘「可省」大字同中线（见下）
        ...
        d.text((26, y + 20), label, font=f_grp, fill=color, anchor="lm")
        # 组头右侧：TOP 标 + 直挂筛选标注（配置可读性）；标实际行数
        # （组不足 top_n 时「TOP5」实画 3 行数字失真，v1.5.37），空组不标
        if fs:
        ...
        for f in fs:
            ...
            for (txt, _), (name, w) in zip(cells, TBL_COLS):
                ...
                if name == "类别":
                    ...
                    # （与价格绿同语义，此前注释写了却没实现）。
                    # 描边/文字引 C_DIRECT/C_TRANSFER（v1.5.44 收编：曾用
            ...
            if qual:
                d.rectangle([12, y, 16, y + rh], fill=(14, 131, 69))
            # 行间 hairline（达标行浅绿线）
        ...
        if pmins:
            # 「各渠道最低」子节（v1.5.40 撤自钉钉文本段：纯行情无链接，
        ...
        y += 18
    if legend_h:
        # 两行图例（v1.5.43）：首行=价格档色+衔接警示色（决策色全部
        # 有解码——橙红 LAY_SHORT 着色的「停X·二段HH:MM」此前无词条，
        # 首见者只能猜）；次行=徽标与比价词条。「真达标/描边绿/擦边」
        # 词面与走势环小注、webui 明细档位同语言（v1.5.47；总长较旧
        # 词面更短，14px 档无降号风险）
        _fit_text(d, "实心绿=真达标 · 描边绿=行情破线 · 琥珀=擦边 · "
                     "橙红=衔接不足",
...
def render_chart(series, title, thresholds=(0, 0), out_path="data/trend.png",
    ...
    # 2x 超采样与明细表 _ScaledDraw 同一模式（v1.5.37）：内部画布
    ...
    if not series:
        ...
        # ——此前仅靠调用方 prepare_round_charts 的 if not series 挡住。
        # 空态统一白卡（v1.5.43：曾灰底裸字与下方双空分支两态两张皮）
        ...
        d.text((20, 20), "暂无数据", font=d.font(21), fill=C_TEXT,
               anchor="lm")   # 与 direct/transfer 空态同 lm 中线锚（v1.5.44）
    ...
    if not vals and not th:
        d.rounded_rectangle([6, 6, W - 7, H - 7], radius=14,
                            fill=DESIGN.CARD, outline=DESIGN.TIP_LINE, width=2)
    ...
    # ▼红=达标回落词条（v1.5.43）：红下箭头 v1.5.40 入图后图面无解释，
    # 首见者只能猜。v1.5.47 词面与 webui 环标逐字同语言（真达标/擦边）
    nx = PAD_L
    f_ring = d.font(13)
    for t, c in (("●实绿", C_QUAL), ("=真达标　", DESIGN.MUTED),
                 ("○细绿", C_QUAL), ("=行情破线　", DESIGN.MUTED),
                 ("○琥珀", C_NEAR),
                 ("=擦边　", DESIGN.MUTED),
                 ("▼红", C_CMP), ("=达标回落", DESIGN.MUTED)):
    ...
    # 网格 + Y 轴刻度（右对齐贴线、行中线锚，v1.5.43——旧 la 锚
    ...
    # 单轮空态提示（v1.5.44 挪到网格后）：原先画在网格之前，横竖网格
    ...
    for idx, color in ((1, C_DIRECT), (2, C_TRANSFER)):
        ...
        # 珠化降密（v1.5.43）：48h/15min ≈192 点/线，r3 珠点把线宽视觉
        ...
        # 实心绿点、行情破线=细绿空心环——两档曾只差 1px 线宽不可分）
        ...
        if th_v:
            ...
            for i, s2 in enumerate(series):
                ...
                prev_qual = qual
                if v <= th_v:
                    ring, rw = C_QUAL, (2 if qual else 1)
                    tier = 2 if qual else 1
                elif v <= th_v * (1 + NEAR_RATIO):
                    ring, rw, tier = C_NEAR, 2, -1
                else:
                    prev_tier = 0
                    continue
                # 状态入场点降密（v1.5.47 与 webui 曲线同律）：整段躺在
                # 破线/擦边带时逐点密铺曾糊成一串环链——仅档位切换点与
                # 末点落环（▼回落箭头仍逐转换绘制）
                if tier != prev_tier or i == n_ser - 1:
                    px, py = xy(i, v)
                    if tier == 2:
                        # ●实心绿=真达标（白描边防曲线穿心）
        ...
        # 下标显式记录（v1.5.43）：series.index 按值相等找下标，同价
        # 同旗标不同时刻会标错点
    ...
    # （v1.5.37）：避让系统原本只见已放置标签、不见固定小注，白底标签
...
def _chart_routes(cfg):
    ...
    扫描端（main._build_users）同样过滤——此前此处是唯一漏口：停用航线的
    历史数据仍被画进日报，用户收到「无渠道明细」空小节+一张过期走势图
    （2026-09-18 用户实锤：SHA→URC 停用后日报仍在报它）。"""
...
def _route_latest_flights(db_path, fc, tc, date, max_age_min=150,
    ...
    try:
        ...
        # 决策字段同步补全（cabin/meal/prate/plane，v1.5.43）：日报
        # 明细次行信息密度与告警池对齐
        Alerter._propagate_fields(out)
    except Exception:
        pass
    # 跨渠道孤低价守卫（v1.5.45，第五消费端）：日报 KPI/明细表/查现价
    # 选台同一防线——2026-09-19 09:33 日报曾在幻影窗口内把 ￥930 推进
...
def _split_market_pool(fs, rc):
    ...
    行情与达标两层语义各自表达、互不混池。"""
    # 脏价守卫（v1.5.49）：非数值/越界价行不入池——曾穿透到 min(key=
    ...
    for f in directs:   # 直飞行同打绿旗：全表「价格绿=达标」同一语义
        ...
        # 恒两档色、与同日报走势环/KPI 两套分档语言（v1.5.43 修复）
    mkt_t = []
    for f in fs:
...
def build_and_push(cfg, logger, notifier, user=""):
    ...
    db_path = _anchored_db(cfg)
    desp = "#### 📈 每日价格日报\n\n"
    # 图例行撤入走势图（v1.5.40：图内点环分档小注与本行同款四档，双份
    # 冗余；无链接信息一律入图的定律）——文本只留口径时间戳。短式
    # （v1.5.43）：原行 47 半角无守卫手机必折行；「档位图例见图内」
    # 与图内小注双份冗余删
    ...
    for (fc, tc, date), url in sorted(charts.items(), key=lambda kv: kv[0]):
        ...
        # 行宽守卫与主链路同法（v1.5.47：长城市对 #### 直落 >20 全角
        # 窄屏折行断点不受控——日期下沉独立引用行）
        ...
        if not directs and not mkt_t:
            ...
            desp += (_fit_line(
                ...
                + f"![走势]({url})\n\n")
            continue
        # KPI 图内化（v1.5.40 定律收口）：行情数字撤文本、进明细表图
        # summary 行（脱离上下文也能看懂当前行情——该参数本就为此设计）；
        # 图生成/上传失败时退回文本 KPI（信息不能跟图一起消失）。
        # 档位词与走势环四档同义：达标/行情破线/距线N%/差￥N
        # （v1.5.46 统一词序「行情破线」=环标小注同词；diff==0 曾不问
        ...
        def _kpi_txt(label, price, th, qual_hit):
            ...
            v1.5.48 单源 core.alerter.kpi_tier_txt（multi 总表图内
        ...
        def _kpi(label, price, th, qual_hit):
            ...
            if th:
                ...
                # 尾注仅严格破线挂（v1.5.49 与主链路 mkt_note 同律）：
        ...
        if fs:
            ...
            # 「各渠道最低」子节补齐（v1.5.43）：告警单/总表已有，日报
            # 此前整篇无渠道分解。数据源=近 2.5 时各平台最新池（有明细
            ...
            for _f2 in (directs + mkt_t):
                ...
                _v2 = _f2.get("price")
                if not isinstance(_v2, (int, float)) or not 0 < _v2 <= 50000:
                    continue
            ...
            _meta = {"layover_min": int(rc.get("transfer_layover_min", 0) or 0),
                     "arrival_max": rc.get("transfer_arrival_max",
                                           "02:00") or "02:00",
                     "plat_mins": _pmins}
            # 日报补同班比价组（v1.5.46）：实时总表已有 compare，日报
            ...
            try:
                ...
                if turl:
                    ...
                    # v1.5.36 后图是明细唯一载体，挂了必须留痕；KPI 同步
                ...
                desp += ("> ⚠️ 明细表生成失败，本节无明细图\n\n"
                         + _kpi_fallback())
        # 小节尾「打开渠道查现价」：日报此前整篇零可点链接，用户想看
        # 某航线只能回翻聊天记录。渠道与主推送同规则（同价指纹优选
        # 去哪儿/携程，无则最低价所在渠道），链接与爬虫主路径同源
        try:
            from types import SimpleNamespace
            _pool = directs + mkt_t
            # 选台基带价合法守卫（v1.5.44）：price=0/异常行曾被 min 选为
            ...
            _plat = (Alerter._pref_platform(
                ...
                if _pool else "")
            # 无合法行不再兜底 qunar（v1.5.47 拔除 v1.5.44 清理漏网：
...
def maybe_daily_report(cfg, logger, notifier, user=""):
    ...
    旧逻辑 now.hour != report_hour 即跳过——服务整点未运行（宕机/重启/
    挂起恢复）就静默丢掉当天日报；改为 hour ≥ report_hour 的当次扫描即
    补推。当天是否已推记在 data/report_state.json（按 user 分 key，值=
    ...
    key = user or "default"
    # 锚定仓库根（v1.5.49 CWD 家族）：CWD 漂移=当日防重发失效同文重推
